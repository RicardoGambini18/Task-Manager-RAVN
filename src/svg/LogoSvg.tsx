import React, { FunctionComponent, ReactElement } from 'react'
import StyledSvg, { SvgProps } from './StyledSvg'

const LogoSvg: FunctionComponent<SvgProps> = ({ width, height, color }): ReactElement => {
  return (
    <StyledSvg
      width={width || 40}
      height={height || 36}
      color={color || 'white'}
      viewBox="0 0 40 36"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        // eslint-disable-next-line max-len
        d="M30.4218 24.5565C35.7216 23.1082 39.6183 18.2592 39.6183 12.5C39.6183 5.71624 34.214 0.194797 27.477 0.00660328V0H8.06627H0L6.69512 8.33114H8.06627V8.33334H27.181C29.4535 8.36636 31.2857 10.2186 31.2857 12.4989C31.2857 14.8002 29.4204 16.6656 27.1194 16.6656H24.0811H13.3913L28.9285 36H39.6172L30.4218 24.5565Z"
        fill="white"
      />
      <path
        // eslint-disable-next-line max-len
        d="M9 36C12.0376 36 14.5 33.5376 14.5 30.5C14.5 27.4624 12.0376 25 9 25C5.96243 25 3.5 27.4624 3.5 30.5C3.5 33.5376 5.96243 36 9 36Z"
        fill="white"
      />
    </StyledSvg>
  )
}

export default LogoSvg
